...
// AWSVMSchedulerStopsdReconciler reconciles a AWSVMSchedulerStopsd object
type AWSVMSchedulerStopsdReconciler struct {
	Client client.Client
	Scheme *runtime.Scheme
	Log    logr.Logger
}

//+kubebuilder:rbac:groups=aws.xyzcompany.com,resources=awsvmschedulerstopsds,verbs=get;list;watch;create;update;patch;delete
//+kubebuilder:rbac:groups=aws.xyzcompany.com,resources=awsvmschedulerstopsds/status,verbs=get;update;patch
//+kubebuilder:rbac:groups=aws.xyzcompany.com,resources=awsvmschedulerstopsds/finalizers,verbs=update
// +kubebuilder:rbac:groups=batch,resources=cronjobs;jobs,verbs=get;list;watch;create;update;patch;delete

// Reconcile is part of the main kubernetes reconciliation loop which aims to
// move the current state of the cluster closer to the desired state.
// TODO(user): Modify the Reconcile function to compare the state specified by
// the AWSVMSchedulerStopsd object against the actual cluster state, and then
// perform operations to make the cluster state reflect the state specified by
// the user.
//
// For more details, check Reconcile and its Result here:
// - https://pkg.go.dev/sigs.k8s.io/controller-runtime@v0.13.0/pkg/reconcile
func (r *AWSVMSchedulerStopsdReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	awsVMScheduler := &awsv1.AWSVMSchedulerStopsd{}
	...
	if err != nil && errors.IsNotFound(err) {
		...
		cron := r.cronJobForAWSVMSchedulerStopsd(awsVMScheduler)
...
func (r *AWSVMSchedulerStopsdReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&awsv1.AWSVMSchedulerStopsd{}).
		Owns(&batchv1.CronJob{}).
		Complete(r)
}

// CronJob Spec
func (r *AWSVMSchedulerStopsdReconciler) cronJobForAWSVMSchedulerStopsd(awsVMScheduler *awsv1.AWSVMSchedulerStopsd) *batchv1.CronJob {

	cron := &batchv1.CronJob{
		ObjectMeta: v1.ObjectMeta{
			Name:      awsVMScheduler.Name,
			Namespace: awsVMScheduler.Namespace,
			Labels:    AWSVMSchedulerStopsdLabels(awsVMScheduler, "awsVMScheduler"),
		},
...
func AWSVMSchedulerStopsdLabels(v *awsv1.AWSVMSchedulerStopsd, tier string) map[string]string {
	return map[string]string{
		"app":                     "AWSVMSchedulerStopsd",
		"AWSVMSchedulerStopsd_cr": v.Name,
		"tier":                    tier,
	}
}
