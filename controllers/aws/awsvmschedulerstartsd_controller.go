...
// AWSVMSchedulerStartsdReconciler reconciles a AWSVMSchedulerStartsd object
type AWSVMSchedulerStartsdReconciler struct {
	Client client.Client
	Scheme *runtime.Scheme
	Log    logr.Logger
}

//+kubebuilder:rbac:groups=aws.xyzcompany.com,resources=awsvmschedulerstartsds,verbs=get;list;watch;create;update;patch;delete
//+kubebuilder:rbac:groups=aws.xyzcompany.com,resources=awsvmschedulerstartsds/status,verbs=get;update;patch
//+kubebuilder:rbac:groups=aws.xyzcompany.com,resources=awsvmschedulerstartsds/finalizers,verbs=update
// +kubebuilder:rbac:groups=batch,resources=cronjobs;jobs,verbs=get;list;watch;create;update;patch;delete

// Reconcile is part of the main kubernetes reconciliation loop which aims to
// move the current state of the cluster closer to the desired state.
// TODO(user): Modify the Reconcile function to compare the state specified by
// the AWSVMSchedulerStartsd object against the actual cluster state, and then
// perform operations to make the cluster state reflect the state specified by
// the user.
//
// For more details, check Reconcile and its Result here:
// - https://pkg.go.dev/sigs.k8s.io/controller-runtime@v0.13.0/pkg/reconcile
func (r *AWSVMSchedulerStartsdReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	awsVMScheduler := &awsv1.AWSVMSchedulerStartsd{}
	...
	if err != nil && errors.IsNotFound(err) {
		...
		cron := r.cronJobForAWSVMSchedulerStartsd(awsVMScheduler)
...
func (r *AWSVMSchedulerStartsdReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&awsv1.AWSVMSchedulerStartsd{}).
		Owns(&batchv1.CronJob{}).
		Complete(r)
}

// CronJob Spec
func (r *AWSVMSchedulerStartsdReconciler) cronJobForAWSVMSchedulerStartsd(awsVMScheduler *awsv1.AWSVMSchedulerStartsd) *batchv1.CronJob {

	cron := &batchv1.CronJob{
		ObjectMeta: v1.ObjectMeta{
			Name:      awsVMScheduler.Name,
			Namespace: awsVMScheduler.Namespace,
			Labels:    AWSVMSchedulerStartsdLabels(awsVMScheduler, "awsVMScheduler"),
		},
...
func AWSVMSchedulerStartsdLabels(v *awsv1.AWSVMSchedulerStartsd, tier string) map[string]string {
	return map[string]string{
		"app":                      "AWSVMSchedulerStartsd",
		"AWSVMSchedulerStartsd_cr": v.Name,
		"tier":                     tier,
	}
}
